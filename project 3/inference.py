        ...
        for p in self.legalPositions:
            if(noisyDistance==None):
                ...
                if emissionModel[trueDistance] > 0:
                  allPossible[p] = self.beliefs[p]*emissionModel[trueDistance]*1.0          
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        number =0
        while(number<numParticles):
         for k in self.legalPositions:
            self.particles.append(k)
            number+=1
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            if noisyDistance==None:
                ...
                if emissionModel[trueDistance]>0:
                    allPossible[p] = self.beliefs[p]*emissionModel[trueDistance]*1.0
                if allPossible[p] == 0.0:
                    count=count+1
        allPossible.normalize()
        if allPossible.totalCount()==0:
            self.initializeUniformly(gameState)    
        elif count!=len(self.legalPositions):
            self.particles=[]
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        allPossible1 = util.Counter()
        for p in self.particles:
            allPossible1[p]+=1.0
        allPossible1.normalize()   
        return allPossible1    
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for e in itertools.product(self.legalPositions,repeat =2):
            particles1.append(e)    
    ...
    def observeState(self, gameState):
        ...
        count = 0
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:#updating particles if noisy distance is 0
                Particles = []
                for p in self.particles:
                    a= self.getParticleWithGhostInJail(p,i)
                    Particles.append(a)
                self.particles = Particles
        for p in self.particles:
            allPossible[p]=1.0
            for i in range(0,len(noisyDistances)):
                if noisyDistances[i] != None:
                    trueDistance = util.manhattanDistance(pacmanPosition,p[i])
                    allPossible[p] *= emissionModels[i][trueDistance]#as per dynamic bayes net formula
                if allPossible[p]==0.0:
                    count = count + 1
               
        if allPossible.totalCount()==0:
            self.initializeParticles()#reinitialize particles with itertools. 
            
        elif count!=self.legalPositions:
            self.beliefs = self.getBeliefDistribution()
            for p in self.particles:
                self.beliefs[p] *= allPossible[p]#beliefs are updated
                allPossible[p] = 1.0#to initialize the counter variable if sum of weights not equal to 0.
            self.beliefs.normalize()    
            self.particles=[]
            for i in range (0, self.numParticles):
              self.particles.append(util.sampleFromCounter(self.beliefs))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newParticle1.append(util.sample(newPosDist))   
            newParticles.append(tuple(newParticle1))    
