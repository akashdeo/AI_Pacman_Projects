...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
       horizontal continouus pixels which are on
       vertical continuous pixels which are on
    ##
    """
    ...
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            p=0
            q=0
            a=datum.getPixel(x,y)
            b=datum.getPixel(x-1,y)
            c=datum.getPixel(x,y-1)
            if a>b:
                p=1
            if a>c:
                q=1
...
def enhancedPacmanFeatures(state, action):
    ...
    capsulelocations = state.getCapsules()
    pos1=successor.getGhostPositions()
    dist=15
    for each in pos1:
        dis=util.manhattanDistance(pos,each)
        if dis<dist:
            dist=dis
    features['dist']=dist+5+(4*len(capsulelocations))+value
    return features
